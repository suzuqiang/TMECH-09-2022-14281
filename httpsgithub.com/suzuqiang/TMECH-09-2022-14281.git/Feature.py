...
class ProNet(torch.nn.Module):
    ...
    def SSLProNetOld1(self):
        ...
        return History
    def GetFeature(self):
        for step, (testinputs,testlabel) in enumerate(self.TestDataSet):
            testinputs,testlabel = testinputs.to(self.device),self.MyDataCenter.GetLabel(testlabel).to(self.device)
            batch = testinputs.size()[0]
            feature = []
            feature.append(self.blocks[0](testinputs).view(batch,-1).cpu().detach().numpy())
            feature.append(self.blocks[1](self.blocks[0](testinputs)).view(batch,-1).cpu().detach().numpy())
            feature.append(self.blocks[2](self.blocks[1](self.blocks[0](testinputs))).view(batch,-1).cpu().detach().numpy())
            feature.append(self.blocks[3](self.blocks[2](self.blocks[1](self.blocks[0](testinputs)))).view(batch,-1).cpu().detach().numpy())
            feature.append(self.blocks[4](self.blocks[3](self.blocks[2](self.blocks[1](self.blocks[0](testinputs))))).view(batch,-1).cpu().detach().numpy())
            feature.append(self.blocks(testinputs).view(batch,-1).cpu().detach().numpy())
            if step == 0:
                Feature = feature
                Label = testlabel.cpu().detach().numpy()
            else:
                for i in range(len(Feature)):
                    Feature[i] = np.concatenate((Feature[i],feature[i]),axis=0)
                Label = np.hstack((Label,testlabel.cpu().detach().numpy()))
        return Feature,Label
    def GetMax(self):
        from sklearn.metrics import confusion_matrix
        from sklearn.metrics import recall_score
        import matplotlib.pyplot as plt
        import os,torch,warnings,sys,time
        import numpy as np
        for step, (testinputs,testlabel) in enumerate(self.TestDataSet):
            testinputs,testlabel = testinputs.to(self.device),self.MyDataCenter.GetLabel(testlabel).to(self.device)
            testfeature = self.blocks(testinputs)
            testlogits = self.GetSoftMax(testfeature,testlabel.size()[0])
            guess = torch.argmax(testlogits,1).cpu().detach().numpy() if step == 0 else np.hstack((guess,torch.argmax(testlogits,1).cpu().detach().numpy()))
            fact = testlabel.cpu().detach().numpy() if step == 0 else np.hstack((fact,testlabel.cpu().detach().numpy()))
        return guess,fact
...
if __name__ == '__main__':
    ...
    Feature,Label = Model.GetFeature()
    Path = args.RandomNum[:-4]+')'+str(args.shot)+')'+\
            str(args.supportusenum)+')'+str(args.PreTrainProNetEpoch)+')'+\
            str(args.PLLEpoch)+')'
    guess,fact = Model.GetMax()
    from sklearn.metrics import confusion_matrix
    from sklearn.metrics import recall_score
    import matplotlib.pyplot as plt
    import os,torch,warnings,sys,time
    ...
    classes.sort()
    confusion = confusion_matrix(guess,fact,normalize='true')
    ...
    plt.ylabel('Ture label',fontsize=10)
    for first_index in range(len(confusion)):
        for second_index in range(len(confusion[first_index])):
            if first_index == second_index:
                plt.text(first_index, second_index, round(confusion[first_index][second_index],2),va = 'center',ha = 'center',color='w')
            else:
                plt.text(first_index, second_index, round(confusion[first_index][second_index],2),va = 'center',ha = 'center')
    plt.show()
    
    # feature2D = Getdigits_proj(Feature,n=2)
    # # feature3D = Getdigits_proj(Feature,n=3)
    # del Feature,Model
    # # test2D = feature2D[0]
    # # test3D = feature3D[4]
    ...
    # for i in range(len(feature2D)):
    #     scatter2D(feature2D[i], Label)
    #     plt.savefig(Path+'digits_tsne-generated'+str(i)+'.png', dpi=120)
    #     plt.show()   
