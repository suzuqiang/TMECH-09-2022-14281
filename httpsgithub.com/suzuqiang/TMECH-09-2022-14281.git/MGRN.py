...
class mGRNDropout(torch.nn.Module):
    def __init__(self, rnn_hidden_size, num_classes, device, input_size_list,
        ...
        # --------------------------------------重点-------------------------------------------------
        self.encoder = ListModule(self,'encoder')
        self.out = nn.Sequential(
            nn.Linear(8*6, 9)#连接全连接到input_size大小
        )
      ...
      #--------------------------------------重点-------------------------------------------------
        for i in range(10):#10分钟，导致需要用10个encoder层。
                self.encoder.append(nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=8*6, nhead=4, dropout=0, dim_feedforward=256), num_layers=2))
        # 分为22个不同的边际网络，隐藏层为6，分为4头，全连接层为256.
        # --------------------------------------重点-------------------------------------------------
    ...
    def forward(self, x):
        ...
        #------------------------------------------重点-----------------------------------------------
        for zhizhen in range(10):
            input_index_beg = 0
            # ------------------------------------------重点-----------------------------------------------
            for i, input_size in enumerate(self.input_size_list):#[4, 4, 2, 8, 4]
                gru = self.marginal_list[i]
                input_index_end = input_size + input_index_beg
                this_x = x[zhizhen, :, input_index_beg:input_index_end]
                this_x = this_x.unsqueeze(0)
                if zhizhen == 0:
                    this_out, this_newgate = gru(this_x, h0)#h0需要拆分为len(input)这么多个
                    h_all[zhizhen].append(this_out)
                    ...
                    this_out, this_newgate = gru(this_x, h_all2[zhizhen-1][0][:,i*6:(i+1)*6])#h0需要拆分为len(input)这么多个
                    #-------------------------------极其重要-----------------------------------
            ...
            # if zhizhen == 0:
            #     h_all[zhizhen] = torch.cat(h_all[zhizhen],dim = 1).unsqueeze(0)
            #     h_now = self.encoder[zhizhen](h_all[zhizhen]).squeeze(0)
            #     h_all2[zhizhen].append(h_now)
            # else:
            #     h_now = h_now.unsqueeze(0)
            #     h_now = self.encoder[zhizhen](h_now).squeeze(0)
            #     h_all.append(h_now)
            h_all[zhizhen] = torch.cat(h_all[zhizhen], dim=1).unsqueeze(0)
            h_now = self.encoder[zhizhen](h_all[zhizhen]).squeeze(0)
            h_all2[zhizhen].append(h_now)
